...
def search_view(request):
    """
    Vue originale de scraping — INCHANGÉE.
    Lance le scraper Jumia sur la query, exporte le CSV, retourne les produits.
    """
    query = request.GET.get("query", "pc portable")

    scraper = JumiaScraper()
    products = scraper.scrape(query)
    scraper.export_to_csv(products, CSV_PATH)          # sauvegarde → même CSV lu par data mining
