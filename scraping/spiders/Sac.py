                    ...
                    try:
                        ...
                        for item in items:
                            ...
                            if title_el:
                                ...
                                # --- Nettoyage de l'Ancien Prix en FLOAT ---
                                old_price_val = 0.0
                                if old_price_el:
                                    raw_old_price = old_price_el.inner_text().strip()
                                    clean_old_price = re.sub(r'[^\d,.]', '', raw_old_price).replace(',', '.')
                                    try: old_price_val = round(float(clean_old_price), 2)
                                    except ValueError: pass
