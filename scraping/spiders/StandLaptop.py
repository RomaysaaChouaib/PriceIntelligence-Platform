...
class LaptopStandJumia:
    ...
    def generate_queries(self, query):
        related = {
            "laptop_stand": [
                ...
                "support aluminium laptop",
                "porte ordinateur portable",
                "rehausseur pc portable",
                "tablette pc portable",
                "support ergonomique pc"
            ]
        }
        # Retourne les requêtes pour laptop_stand, sinon utilise le mot passé en paramètre
        return related.get(query.lower().replace(" ", "_"), related.get("laptop_stand", [query]))
    ...
    def scrape(self, query="laptop_stand", max_pages=10): 
        products = []
        seen_links = set()
        seen_titles = set()

        # Mots-clés requis pour confirmer qu'il s'agit bien d'un sac
        pc_keywords =["support", "stand", "rehausseur", "refroidisseur", "cooler", "tablette", "porte", "base"]
        
        # Mots-clés à exclure (accessoires qui se glissent souvent dans ces recherches)
        excluded = [
            "sac", "sacoche", "housse", "pochette", "backpack",
            "sticker", "autocollant", "cable", "câble", "batterie",
            "clavier", "souris", "ecran", "antivol", "film"
        ]
        ...
        for q_idx, search_word in enumerate(queries, 1):
            ...
            for page in range(1, max_pages + 1):
                ...
                for item in items:
                    ...
                    # Filtre de prix ajusté à 30 DH minimum pour éviter les gadgets à 10 DH
    ...
    def export_to_csv(self, products, filename="jumia_laptop_stand.csv"):
        if not products:
...
class LaptopStandAmazon:
    ...
    def generate_queries(self, query):
        ...
        "support pc portable ergonomique",
        "support ventilé pc portable",
        "refroidisseur pc portable",
        "support macbook pro",
        "support ordinateur portable réglable",
        "support vertical pc portable",
        "porte ordinateur portable",
        "base de refroidissement pc",
        "support pc portable gamer",
        "support tablette et ordinateur",
        "rehausseur ecran pc portable"
    ]
        }
        return related.get(query.lower().strip(), [query])

    def scrape(self, base_query="laptop_stand", max_pages=5): # J'ai réduit max_pages par défaut pour éviter le ban IP
        results = []
    ...
    def export_to_csv(self, products, filename="amazon_laptop_stand.csv"):
        if not products:
...
class LaptopStandAliexpress:
    ...
    def generate_queries(self, query):
        ...
        "base de refroidissement pc",
        "notebook stand holder",
        "support pc portable pliable",
        "ergonomic laptop holder",
        "rehausseur pc portable",
        "laptop cooling pad",
        "support macbook air pro",
        "porte ordinateur réglable",
        "vertical laptop stand",
        "support pc portable bureau",
        "laptop riser",
        "support ventilé ordinateur"
    ]
        }
        
        normalized_query = query.lower().strip().replace(" ", "_")
        return related.get(normalized_query, related.get("laptop_stand", [query]))
    ...
    # SCRAPE
    # ------------------------------------------------------------------
    def scrape(self, base_query="laptop_stand", max_pages=10):
        results = []
        queries = self.generate_queries(base_query)
                        ...
                        for item in items:
                           ...
                           # --- PRIX (LOGIQUE CORRIGÉE CIBLANT UNIQUEMENT .lw_el) ---
                            ...
                            results.append({
                                ...
                                "currency": currency,
                                "brand": title_text.split()[0] if title_text.split() else "Inconnu",
                                "category": "laptop_stand", # MISE À JOUR ICI
                                "source": self.source_name,
                                "link": product_link,
                                "image": image_url,
                                "search_query": search_word,
    ...
    def export_to_csv(self, products, filename="aliexpress_laptop_stand.csv"): # MISE À JOUR ICI
