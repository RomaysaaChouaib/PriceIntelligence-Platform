    ...
    <div>
      ...
      {mode === "scrape" && (
        <div className="pip-card" style={{ marginBottom: 20, padding: 20 }}>
          <p style={{ marginBottom: 15, fontWeight: "bold", color: "#1e293b" }}>Sélectionnez la catégorie à scraper :</p>
          <div style={{ display: "flex", gap: "10px", marginBottom: "20px", maxWidth: "400px" }}>
            <button onClick={() => setScrapeCategory("laptop")} style={categoryBtnStyle(scrapeCategory === "laptop")}>💻 Laptops</button>
            <button onClick={() => setScrapeCategory("accessoire")} style={categoryBtnStyle(scrapeCategory === "accessoire")}>🎧 Accessoires</button>
          </div>
          <div className="scrape-buttons-group" style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 15 }}>
            {scrapeCategory === "laptop" ? (
              <>
                <button onClick={() => runScrape("jumia")} disabled={loading || polling} style={scrapeBtnStyle("#f97316")}>Scraper Jumia</button>
                <button onClick={() => runScrape("amazon")} disabled={loading || polling} style={scrapeBtnStyle("#232f3e")}>Scraper Amazon</button>
                <button onClick={() => runScrape("aliexpress")} disabled={loading || polling} style={scrapeBtnStyle("#e62e04")}>Scraper AliExpress</button>
                <button onClick={() => runScrape("all")} disabled={loading || polling} style={scrapeBtnStyle("#1e293b")}>🔥 Tout Scraper</button>
              </>
            ) : (
              <>
                <button onClick={() => runScrape("souris")} disabled={loading || polling} style={scrapeBtnStyle("#3b82f6")}>🖱️ Souris</button>
                <button onClick={() => runScrape("laptop_stand")} disabled={loading || polling} style={scrapeBtnStyle("#3b82f6")}>🏗️ Support PC</button>
                <button onClick={() => runScrape("cooling_pad")} disabled={loading || polling} style={scrapeBtnStyle("#3b82f6")}>❄️ Refroidisseur</button>
                <button onClick={() => runScrape("sac_laptop")} disabled={loading || polling} style={scrapeBtnStyle("#3b82f6")}>🎒 Sac PC</button>
                <button onClick={() => runScrape("usb")} disabled={loading || polling} style={scrapeBtnStyle("#3b82f6")}>💾 Clé USB</button>
              </>
            )}
          </div>
          <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: "15px" }}>
            <button onClick={handleStop} style={scrapeBtnStyle("#ef4444")}><X size={18} style={{ marginRight: "8px" }} /> STOP</button>
          </div>
        </div>
      )}
      ...
      {scrapeMsg && !polling && progress === 0 && (
        <div style={{ padding: "12px 16px", borderRadius: 8, marginBottom: 14, fontWeight: 500, fontSize: 13, color: scrapeMsg.startsWith("✅") ? "#166534" : "#991b1b", background: scrapeMsg.startsWith("✅") ? "#dcfce7" : "#fee2e2", border: `1px solid ${scrapeMsg.startsWith("✅") ? "#bbf7d0" : "#fecaca"}` }}>
          {scrapeMsg}
        </div>
      )}
      ...
      <div className="pip-status-bar" style={{ marginBottom: 15 }}>
        ...
        <span style={{ marginLeft: 10, padding: "4px 8px", borderRadius: 4, background: "#e2e8f0", fontSize: 12, fontWeight: 600 }}>
          {mode === "csv" ? "Fichier CSV" : mode === "db" ? "Base de données" : mode === "db_accessoire" ? "Accessoires" : mode === "alerts" ? "🔥 Alertes" : "Scraping"}
        </span>
      </div>
